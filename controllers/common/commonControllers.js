    ...
    // 통합 업로드
    {
        ...
        callback : async ({request, params}) => {
           try {
                ...
                if(!isValidFile(files, allowedFiles)){ // 파일 업로드 허용 체크
                    return {
                        message: 'noAllowFile',
                        success: false
                    }
    ...
    // 문서 업로드
    {
        ...
        callback : async ({request, params}) => {
           try {
                ...
                if(!isValidFile(files, allowedDocuments)){ // 문서 파일 업로드 허용 체크
                    return {
    ...
    // group_id 수정
    {
        ...
        callback : async ({request, params}) => {
           try {
                ...
                const result = await sendQuery("update file set file_group_id = $1 where file_group_id = $2", 
